...
import _tmpl from "./resources.html";
import { getNearbyNotableObservations, getNotableSightingsByLocation, getNearbyObservations } from 'data/ebirdService';
import { getStates, getCounties } from 'data/restdbService';
class Resources extends LightningElement {
  constructor(...args) {
    super(...args);
    this.localSightings = void 0;
    this.stateSightings = void 0;
    this.nearbySightings = void 0;
    this.countySightings = void 0;
    this.selectedSightings = [];
    this.stateOptions = [];
    this.stateLabel = 'Virginia';
    this.countyLabel = 'Accomack';
    this.dropdown = 'Local';
  ...
  getCountyOptions() {
    ...
    if (counties) {
      ...
      getCounties().then(response => {
        ...
        sessionStorage.setItem('counties', JSON.stringify(this.countyOptions));
      }).catch(error => {
        console.log(error);
      });
    }
  ...
  getCounties(state) {
    this.filteredCounties = this.countyOptions.filter(item => item.value.startsWith(state));
  }
  ...
  handleSelectedStateChange() {
    this.stateLabel = this.stateOptions.find(item => item.value === this.setting.state).label;
    this.getStateNotableSightings();
  }
  handleSelectedCountyChange() {
    this.countyLabel = this.countyOptions.find(item => item.value === this.setting.county).label;
    this.getCountyNotableSightings();
  }
  handleStateChange(event) {
    this.getCounties(event.detail.value);
    this.template.querySelector('.county').value = this.filteredCounties[0].value;
  }
  handleListClick() {
...
_registerDecorators(Resources, {
  fields: ["localSightings", "stateSightings", "nearbySightings", "countySightings", "selectedSightings", "stateOptions", "stateLabel", "countyLabel", "dropdown", "showFilterModal", "countyOptions", "setting", "filteredCounties", "lastUpdated"]
});
export default _registerComponent(Resources, {
  tmpl: _tmpl
});